<template>
  <div class="water-profile-shell" :aria-busy="loading || refreshing">
    ...
    <div v-show="!loading && !isEmpty" ref="chartElement" class="water-profile-chart" role="img"
         aria-label="最新时刻节点水位空间剖面"></div>
...
.water-profile-shell {
  ...
  overflow: hidden;
  border: 1px solid rgba(54, 135, 190, .2);
  border-radius: 6px;
...
.profile-loading span, .profile-refreshing span {
  width: 12px;
  height: 12px;
  border: 2px solid rgba(57, 246, 255, .25);
  border-top-color: #39f6ff;
  border-radius: 50%;
...
.profile-error {
  ...
  z-index: 4;
  padding: 5px 9px;
  border: 1px solid rgba(255, 117, 104, .38);
  border-radius: 4px;
  color: #ffaaa2;
  background: rgba(60, 17, 20, .88);
  font-size: 12px;
}
