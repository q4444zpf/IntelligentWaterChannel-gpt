  ...
  <Teleport to="body">
    <div class="modal-backdrop" @click.self="$emit('close')">
      <section class="panel detail-panel alarm-modal" role="dialog" aria-modal="true" aria-labelledby="alarm-modal-title">
        ...
        <dl><dt>报警编号</dt><dd>{{ alarm.code }}</dd><dt>报警设备</dt><dd>{{ alarm.device }}（{{ alarm.deviceType }}）</dd><dt>所属位置</dt><dd>{{ alarm.location }}</dd><dt>报警等级</dt><dd><span class="tag" :class="{ 'danger-tag': alarm.level === '严重' }">{{ alarm.level }}</span></dd><dt>报警时间</dt><dd>{{ alarm.warnTime || alarm.time }}</dd><dt>报警信息</dt><dd>{{ alarm.message }}</dd></dl>
